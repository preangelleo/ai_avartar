...
def clear_chat_history(chat_id, message_id):
    ...
    for i in range(message_id, message_id - 20, -1):
        response = requests.get(f'https://api.telegram.org/bot{TELEGRAM_BOT_RUNNING}/deleteMessage?chat_id={chat_id}&message_id={str(i)}')
...
def send_img_to_all(img_file, description='', send_from='None'):
    ...
    try:
        ...
        # 通知 bot owner 发送成功
        send_msg(f"{user_nick_name}, 我已经把图片发送给所有 {len(from_ids)} 个用户了啦, 使命必达, 欧耶 😎!", BOTOWNER_CHAT_ID)
...
def send_msg_to_all(msg_text):
    ...
    try:
        ...
        # 通知 bot owner 发送成功
        send_msg(f"{user_nick_name}, 我已经把消息发送给所有 {df.shape[0]} 个用户了.", BOTOWNER_CHAT_ID)
...
def send_file_to_all(file):
    ...
    try:
        ...
        # 通知 bot owner 发送成功
        send_msg(f"{user_nick_name}, 我已经把 {file} 发送给所有 {df.shape[0]} 个用户了.", BOTOWNER_CHAT_ID)
...
def local_bot_msg_command(tg_msg):
    ...
    if 'text' not in tg_msg['message']: 
        ...
        if 'document' in tg_msg['message']:
            try:
                ...
                if file_name in ['dialogue_tone.xls', 'system_prompt.txt'] and from_id not in BOT_OWNER_LIST: return
    ...
    if msg_is_inproper(msg_text): 
        ...
        if r: 
            ...
            return print(f"BLACKLISTED: {blacklisted_alert}")
    ...
    # Welcome and help
        ...
        except: return send_msg(f"{user_nick_name}, 你输入的 {MSG_SLT[1]} 不是整数, 请重新输入哈.", chat_id)
        ...
        return send_msg(f"{user_nick_name}, 我已经把免费用户每月的免费对话次数设置为 {MSG_SLT[1]} 次了, 系统参数表也更新了, 请放心, 参数立刻生效 😘", chat_id)
        ...
        return send_msg(f"{user_nick_name}, 我已经把每月的收费金额设置为 {MSG_SLT[1]} 美元了, 系统参数表也更新了, 但是需要后台重启服务才能生效, 请联系 @laogege6 帮你重启吧 😘", chat_id)
        ...
        with lock: refill_teaser = ' '.join(MSG_SLT[1:])
    ...
    elif MSG_SLT[0] in ['check_refill_teaser', 'crt', '/check_refill_teaser', '/crt'] and chat_id in BOT_OWNER_LIST: return send_msg(f"{user_nick_name}, 以下是当前正在使用的的提醒用户充值的 REFILL_TEASER:\n\n{refill_teaser}", chat_id)
    ...
    elif len(msg_text) <= 2 or msg_text in reply_emoji_list:
...
if __name__ == '__main__':
    ...
    while True:
        ...
        try:
            ...
            update_thread.start()
            time.sleep(1)
        except Exception as e: print(f"ERROR: {i} check_local_bot_updates() failed: {e}")