...
MSG_PER_SECOND = 1
LOAD_TEST_DURARION_SECONDS = 1
SEND_MSG_INTERVAL_SECOND = 1. / MSG_PER_SECOND

TESTEE_BOT_ID = '500840430045609984'
TESTEE_BOT_NAME = 'demo_bot'

LOAD_TEST_FANBOOK_BOT_TOKEN = '43bb042a92d99a223945c6d7c184e21be805c69d74ffa370d909e5fe7c80cdef413d5ef002d6b4b65aa84eb3e0097e65'
LOAD_TEST_FANBOOK_SEND_MSG_URL = f'https://a1.fanbook.mobi/api/bot/{LOAD_TEST_FANBOOK_BOT_TOKEN}/sendMessage'  # noqa
LOAD_TEST_CHAT_ID = '508078778074116097'
LOAD_TEST_FANBOOK_CLIENT_ID = '508069176494133248'
BASE_URL = 'https://a1.fanbook.mobi/api'
GET_USER_TOKEN_TIMEOUT_COUNT = 3
FANBOOK_GET_ME_URL = f'{BASE_URL}/bot/{LOAD_TEST_FANBOOK_BOT_TOKEN}/getMe'
DEVICE_ID = f'bot{LOAD_TEST_FANBOOK_CLIENT_ID}'
...
super_str = base64.b64encode(header_map.encode('utf8')).decode('utf8')  # noqa  # noqa
LOAD_TEST_ADDR = f'wss://gateway-bot.fanbook.mobi/websocket?id={get_user_token()}&dId={DEVICE_ID}&v={FANBOOK_VERSION}&x-super-properties={super_str}'
...
async def send_msg_async(msg: str, chat_id):
    ...
    payload = {'chat_id': int(chat_id), 'text': msg, 'desc': msg, 'mentions': [TESTEE_BOT_ID]}
...
async def handle_heart_beat(ws):
    asyncio.create_task(send_ping(ws))


async def main():
    try:
        async with websockets.connect(LOAD_TEST_ADDR) as ws:
            await handle_heart_beat(ws)
            total_msg_accum = 0
            tasks = []
            while total_msg_accum < LOAD_TEST_DURARION_SECONDS * MSG_PER_SECOND:
                ...
                tasks.append(
                    asyncio.create_task(send_msg_async('${@!' + TESTEE_BOT_ID + '} ' + random.choice(QUESTIONS_LIST), LOAD_TEST_CHAT_ID))
                )
                total_msg_accum += 1
