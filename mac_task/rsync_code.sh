...
do
    ...
    rsync -avz --exclude=".DS_Store" --exclude="__pycache__" --exclude="test_inlocal.py" --exclude="files/audio/clone_voice" --exclude="files/bing_search" --exclude=".chroma/" "$folder_name/tg" root@$(cat "$folder_name/configuration.json" | jq -r '.UBUNTU_SERVER_IP_ADDRESS'):/root/
