...
from src.bot.bot_branch.payment_branch.crpto.utils import (
    get_transactions_info_by_hash_tx,
...
def create_news_and_audio_from_bing_search(bot, query, chat_id):
    ...
    text_cn = chat_gpt_regular(
        f"{translate_report_prompt}{text_contents}",
...
class TextBranch(BotBranch):
    ...
    def handle_single_msg(self, msg, bot):
        ...
        logging.info(
            f"IGNORE: {msg.user_title} {msg.from_id}: {msg_text}"
            if msg.should_be_ignored
            else f"LEGIT: {msg.user_title} {msg.from_id}: {msg_text}"
        )
        ...
        if msg_text.lower().startswith('http'):
            if len(msg_text) < 10 or not '/' in msg_text or not '.' in msg_text:
                return
            ...
            try:
                ...
                query = (
                    ' '.join(MSG_SPLIT[1:])
                    if MSG_LEN > 1
                    else "请提炼总结一下此人的 Profile。只需回复内容, 不需要任何前缀标识。"
                    if 'linkedin' in msg_lower
                    else "请为该页面写一个精简但有趣的中文 Tweet。只需回复内容, 不需要任何前缀标识。"
                )
                if 'linkedin' in msg_lower:
                    bot.send_msg(
        ...
        # 用户可以通过 save_chat_history /from_id 指令来保存聊天记录
            ...
            MSG_SPLIT[0] in ['save_chat_history', '/save_chat_history', 'sch', '/sch']
        ...
        # Welcome and help
            ...
            if msg_text in ['/start', 'help', '/help', 'start']:
                ...
                bot.send_img(
                    msg.chat_id,
                    Params().avatar_command_png,
                    description=f'任何时候回复 /help 都可以看到这张图片哦 😁',
                )
        ...
        # 用户主动发起申请成为 vip (永久免费)用户
            ...
            # 通知用户申请发送成功
            bot.send_msg(
                f"{msg.user_nick_name}, 你的 VIP 申请已经发送给 @{bot.bot_owner_name} 了, 请耐心等待老板审批哦 😘",
                msg.chat_id,
            )
            # 给 bot onwer 发送申请消息
            return bot.send_msg(
        ...
        # 提交用户自己的 elevenlabs_api_key
            ...
            elevenlabs_api_key = (
                msg_text.replace('/', '').replace('elevenlabs_api_key', '').strip()
            )
        ...
        # /write_story
            ...
            return chat_gpt_write_story(
                ...
                msg.chat_id,
                msg.from_id,
                story_prompt_from_user,
                gpt_model=Params().OPENAI_MODEL,
            )
        ...
        # /read_story
            ...
            special_password_temp = ''.join(
                random.sample(string.ascii_letters + string.digits + '@$-%^&_*', 17)
            )
            ...
            try:
                ...
                bot.send_msg(
                    f"ERROR: local_bot_msg_command() create_midjourney_prompt() FAILED: \n\n{e}",
                    msg.chat_id,
                )
            return
        ...
        # chatpdf function
            ...
            try:
                reply = chat_gpt_regular(
                    ...
                    use_model=Params().OPENAI_MODEL,
                )
                bot.send_msg(reply, msg.chat_id)
            except Exception as e:
                bot.send_msg(
                    f"对不起{msg.user_nick_name}, 刚才我的网络断线了, 没帮你修改好. 你可以重发一次吗? 😭",
                    msg.chat_id,
                )
            return
        ...
        # translate chinese to english and then generate audio with my voice
            ...
            or '投资异类' in msg_text
            or '/投资异类' in msg_text
        ) and bot.bot_name.lower() in ['leowang_bot']:
            if MSG_LEN == 1:
                return bot.send_msg(
            ...
            if (
                ...
                try:
                    with Params().Session() as session:
                        ...
                        session.commit()
                        bot.send_msg(
                            f"{msg.user_nick_name}, 我已经删除所有用户的聊天记录, 大家可以重新开始跟我聊天了。😘",
                            msg.chat_id,
                        )
                except Exception as e:
                    logging.error(
            ...
            try:
                with Params().Session() as session:
                    stmt = (
                        sqlalchemy.update(ChatHistory)
                        .values(msg_text=None)
                        .where(ChatHistory.msg.from_id == msg.from_id)
                    )
                    session.execute(stmt)
                    session.commit()
                    bot.send_msg(
                        f"{msg.user_nick_name}, 我已经删除你的聊天记录, 你可以重新开始跟我聊天了。😘",
                        msg.chat_id,
                    )
            except Exception as e:
                logging.error(
        ...
        # 为用户输入的内容生成音频并发送
            ...
            bot.send_msg(
                f"好嘞 {msg.user_nick_name}, 我帮你把以下内容转换成语音, 请稍等 1 分钟哦 😁\n\n{content}",
                msg.chat_id,
            )
            try:
