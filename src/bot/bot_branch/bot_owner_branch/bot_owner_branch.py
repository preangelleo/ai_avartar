    ...
    def handle_single_msg(self, msg, bot):
        ...
        if MSG_SPLIT[0] in ['mybots'] or msg.msg_text in ['/mybots']:
            ...
            try:
                ...
                if msg.from_id in get_unique_from_id_list():
                    ...
                    bot.send_msg(
                        f"{msg.user_nick_name}, msg.from_id {msg.from_id} 没有聊天记录, 😘",
                        msg.chat_id,
                    )
            except Exception as e:
                logging.error(
                    f"local_bot_msg_command() get_user_chat_history() FAILED: \n\n{e}"
                )
            return

        # 用 vip 命令设置用户成为 VIP, 当 msg_lower 以 /vip, vip, /vip_, vip_, /v, v, /v_, v_ 开头时, 会触发这个命令, 而 msg_lower 中的数字部分是 from_id
        elif (
            msg_lower.startswith('/vip')
            or msg_lower.startswith('vip')
            or msg_lower.startswith('/v')
            or msg_lower.startswith('v')
        ):
            user_from_id = (
            ...
            if user_from_id and user_from_id.isdigit():
                ...
                if msg.from_id in get_unique_from_id_list():
                    ...
                    if r:
                        ...
                        bot.send_msg(
                            f"{msg.user_nick_name}, 我已经把你设置为 VIP 了, 你可以跟我永久免费聊天了. 😘",
                            user_from_id,
                        )
                        return bot.send_msg(
        ...
        # Remove user from VIP list
            ...
            if user_from_id and user_from_id.isdigit():
                ...
                if r:
                    return bot.send_msg(
                        f"msg.from_id: {user_from_id} 已被成功移出 VIP 列表!", msg.chat_id
                    )
                else:
                    return bot.send_msg(
                        f"msg.from_id: {user_from_id} 本来就不在 VIP 列表中哈。", msg.chat_id
                    )
            ...
            if vip_list_with_hint_text:
                ...
                if vip_count < 11:
                    ...
                    bot.send_file(
                        msg.chat_id, file_name, description=f"您的 {vip_count} 位 VIP 用户列表"
                    )
        ...
        # 发送最新的 user_commands 给用户
            ...
            group_send_message_info = (
                f"{DEAR_USER}, /commands 列表更新咯 😙: \n{user_commands}"
            )
            ...
            try:
                ...
                if r:
                    ...
                    bot.send_msg(
                        f"msg.from_id: {msg.from_id_to_blacklist} 已被成功加入黑名单并已经发消息告知.",
                        msg.chat_id,
                    )
            except Exception as e:
                logging.error(
                    f"local_bot_msg_command() set_user_blacklist() FAILED: \n\n{e}"
                )
            return
            ...
            try:
                ...
                if r:
                    bot.send_msg(
                        f"{msg.user_nick_name}, 我已经把你从黑名单中移除了, 你可以继续跟我聊天了. 😘",
                        msg.from_id_to_remove,
                    )
                    bot.send_msg(
                        f"msg.from_id: {msg.from_id_to_remove} 已被成功移出黑名单!", msg.chat_id
                    )
            except Exception as e:
                logging.error(
                    f"local_bot_msg_command() remove_user_blacklist() FAILED: \n\n{e}"
                )
            return
            ...
            try:
                ...
                return bot.send_msg(
                    f"{msg.user_nick_name}, 你输入的 {MSG_SPLIT[1]} 不是整数, 请重新输入哈.",
                    msg.chat_id,
                )
            ...
            try:
                ...
                return logging.error(
                    f"local_bot_msg_command() update_owner_parameter() FAILED: \n\n{e}"
                )
            ...
            bot.send_img(
                msg.chat_id,
                'files/images/group_send_image_pc.png',
                description='电脑上是这样色儿的 😚',
            )
            bot.send_img(
                msg.chat_id,
                'files/images/group_send_image_phone.PNG',
                description='手机上是这样色儿的 😉',
            )
            return
