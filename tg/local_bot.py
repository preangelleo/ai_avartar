...
if place_holder:
    ...
    user_system_prompt_file = default_system_prompt_file if os.path.isfile(
        default_system_prompt_file) else system_prompt_backup_file
...
def send_img_to_all(img_file, description='', bot_owner_chat_id=BOTOWNER_CHAT_ID):
    ...
    try:
        df = pd.read_sql_query(f"SELECT DISTINCT `chat_id` FROM `avatar_chat_history` WHERE `black_list` = 0", engine)
    except Exception as e:
    ...
    try:
        ...
        for i in range(len(from_ids)):
            ...
            try:
                send_img(from_id, img_file, description)
            except Exception as e:
...
def send_audio_to_all(audio_file, bot_owner_chat_id=BOTOWNER_CHAT_ID):
    ...
    try:
        ...
        # 通知 bot owner 发送成功
        send_msg(f"{user_nick_name}, 我已经把 {audio_file.split('/')[-1]} 发送给所有 {df.shape[0] - 1} 个用户了.",
                 bot_owner_chat_id)
    except Exception as e:
...
def local_bot_msg_command(tg_msg):
    ...
    if 'text' not in tg_msg['message']:
        ...
        if 'document' in tg_msg['message']:
            try:
                ...
                if caption and caption.split()[0].lower() in ['group_send_file', 'gsf', 'group send file']:
                    ...
                    send_msg(f'{user_nick_name}我收到了你发来的文件, 请稍等 1 分钟, 我马上把这个文件发给所有人 😁...',
                ...
                if file_name.endswith('.txt') and file_name != 'system_prompt.txt': loader = TextLoader(save_file_path,
                                                                                                        encoding='utf8')
                if file_name.endswith('.docx') or file_name.endswith('.doc'): loader = UnstructuredWordDocumentLoader(
                    save_file_path)
                if file_name.endswith('.pptx') or file_name.endswith('.ppt'): loader = UnstructuredPowerPointLoader(
                    save_file_path)
                ...
                if loader:
                    ...
                    send_msg(
                        f"{user_nick_name}, 我收到你发来的 {file_name[-4:].upper()} 文档了, 如果想要了解本文档的相关内容, 可以使用 doc 命令前缀加上你的问题, 我会帮你通过矢量数据进行语义搜索, 找到答案。注意, doc 命令后面需要有空格哦 🙂. 现在我先帮你简单看一下这个文档是说什么的. 请稍等 1 分钟哈。🤩",
                        chat_id)
                    ...
                    if r:
                        send_msg(
                            f"{user_nick_name}, 我收到你发来的 dialogue_tone.xls 文档了, 我已经妥善保存, 下一次聊天的时候, 我会按照新文件的指示来应对聊天风格哈, 放心, 我很聪明的 🙂!",
                            chat_id)
                    else:
                        send_msg(
                            f"{user_nick_name}, 我收到你发来的 dialogue_tone.xls 文档了, 但是我处理不了, 请你检查一下格式是否正确哈, 然后再发一次给我 😮‍💨",
                            chat_id)
                elif file_name == 'system_prompt.txt':
                    r = insert_system_prompt_from_file(file_path='files/system_prompt.txt')
                    if r:
                        send_msg(
                            f"{user_nick_name}, 我收到你发来的 system_prompt.txt 文档了, 我已经妥善保存, 下一次聊天的时候, 我会按照新的 System Prompt 要求来定位我自己, 放心, 我很聪明的 🙂!",
                            chat_id)
                    else:
                        send_msg(
                            f"{user_nick_name}, 我收到你发来的 system_prompt.txt 文档了, 但是我处理不了, 请你检查一下格式是否正确哈, 然后再发一次给我 😮‍💨",
                            chat_id)
            except Exception as e:
        ...
        if 'audio' in tg_msg['message']:
            ...
            if audio_caption and audio_caption.split()[0].lower() in ['group_send_audio',
                                                                      'gsa'] and chat_id in BOT_OWNER_LIST:
    ...
    msg_text = ' '.join(
        [tg_msg['message'].get('text', ''), tg_msg['message']['reply_to_message'].get('text')]) if 'reply_to_message' in \
                                                                                                   tg_msg[
    ...
    # 用户可以通过 save_chat_history /from_id 指令来保存聊天记录
        ...
        if os.path.isfile(file_path):
            ...
            send_msg(f"{user_nick_name}, 我没有找到你的聊天记录, , 你应该从来没跟我好好聊过吧 😅\n\nP.S. {help_info}",
                     chat_id)
        return
    ...
    # 用户主动发起申请成为 vip (永久免费)用户
        ...
        # 通知用户申请发送成功
        send_msg(f"{user_nick_name}, 你的 VIP 申请已经发送给 @{TELEGRAM_USERNAME} 了, 请耐心等待老板审批哦 😘", chat_id,
                 parse_mode='', base_url=telegram_base_url)
        # 给 bot onwer 发送申请消息
        return send_msg(
    ...
    # 提交用户自己的 elevenlabs_api_key
        ...
        if not elevenlabs_api_key: return send_msg(eleven_labs_apikey_retrieve_guide, chat_id, parse_mode='',
                                                   base_url=telegram_base_url)
        r = check_and_save_elevenlabs_api_key(elevenlabs_api_key, from_id)
        if r: generate_clone_voice_audio_with_eleven_labs(eleven_labs_english_tranning_text, from_id, user_title,
    ...
    # update_elevenlabs_user_ready_to_clone_to_0(from_id) if msg_text in ['/confirm_my_voice', 'confirm_my_voice'] else None
        ...
        if is_english(content):
            generate_clone_voice_audio_with_eleven_labs(content, from_id, user_title, folder='files/audio/clone_voice')
        else:
            send_msg(
    ...
    elif MSG_SPLIT[0] in ['/check_bill', 'check_bill', '/check_payment', 'check_payment', 'check_bill', '/check_bill',
                          'check_payment_status', '/check_payment_status', '/check_bill_status', 'check_bill_status']:
        # 从数据库中读出该 from_id 对应的收款 eth address
        try:
            ...
            if next_payment_time_dict:
                ...
                send_msg(f"{user_nick_name}, 你下一次交公粮的时间应该是 {next_payment_time}, 你就是我最爱的人 💋💋💋 ...",
                         chat_id, parse_mode='', base_url=telegram_base_url)
            else:
                address = generate_eth_address(user_from_id=from_id)
                send_msg(
    ...
    # chatpdf function
    elif MSG_SPLIT[0] in ['pdf', 'doc', 'txt', 'docx', 'ppt', 'pptx', 'url', 'urls', '/pdf', '/doc', '/txt', '/docx',
                          '/ppt', '/pptx', '/url', '/urls']:
        if MSG_LEN == 1: return send_msg(
    ...
    # 为用户输入的内容生成音频并发送
    elif MSG_SPLIT[0] in ['make_voice', '/make_voice', 'generate_audio', '/generate_audio', 'gv', '/gv', 'make_audio',
                          '/make_audio', 'ma', '/ma', 'mv', '/mv', 'ga', '/ga', 'generate_voice', '/generate_voice']:
        if MSG_LEN == 1: return send_msg(
    ...
    # 查询以太坊链上交易 Transaction Hash
            ...
            if user_from_id and user_from_id.isdigit():
                ...
                if from_id in get_unique_from_id_list():
                    ...
                    if r:
                        ...
                        send_msg(f"{user_nick_name}, 我已经把你设置为 VIP 了, 你可以跟我永久免费聊天了. 😘",
                                 user_from_id)
                        return send_msg(
        ...
        # Remove user from VIP list
            ...
            if user_from_id and user_from_id.isdigit():
                ...
                if r:
                    return send_msg(f"from_id: {user_from_id} 已被成功移出 VIP 列表!", chat_id)
                else:
                    return send_msg(f"from_id: {user_from_id} 本来就不在 VIP 列表中哈。", chat_id)
        ...
        # 发送最新的 user_commands 给用户
            ...
            with lock:
                MessageThread.free_user_free_talk_per_month = free_talk_limit
            ...
            return send_msg(f"{user_nick_name}, 以下是当前正在使用的的提醒用户充值的 REFILL_TEASER:\n\n{refill_teaser}",
                            chat_id)
...
if __name__ == '__main__':
    ...
    while True:
        ...
        try:
            ...
            logging.error(f"{i} check_local_bot_updates() failed: {e}")
